...
local function OnCharacterChange(NewCharacter: Instance & Model):()
	if ProximityService.PlayerIsOnMobile then return end
...
function GenerateSerealNumber()
	local Number = RandomNumber:NextNumber(1000,1999)

	if Sereals[Number] then
		Number = GenerateSerealNumber()
	end
...
function ProximityService.new(ProximityParamters: ProximityParamters):(Proximity)
	...
	local Proximity = setmetatable({}, {
		ProximityStoreValues = {IsOnArea = false;CanContinue = true; CancelAction = SignalService()};
	...
	Proximity.PromptGui, Proximity.Animations, Proximity.SizeCompletedCicle, Proximity.MobileHold  = InterfaceCreator.Create(ProximityService.PlayerIsOnMobile,ProximityParamters.KeyCode.Name, ProximityParamters)
	...
	Proximity.Name = tostring(GenerateSerealNumber())
	...
	if Proximity.MobileHold then
		Proximity.MobileHold:Connect(function(Bouce: boolean):()
			if Bouce then
				Proximity:Hold()
			else
				Proximity:UnHold()
...
function ProximityService:Open():()
	...
	if not Proximity.CanContinue then return end
